...
harn migrate 0.5.0 --apply    # 执行已注册的迁移补丁
...
`harn verify` 会在 `.harness/state/last-verify.json` 写入摘要（不含密钥）。`doctor` 在未运行、上次失败或超过约 7 天未验证时会提示。`.harness/state/` 可由团队选择提交或加入 `.gitignore`。
